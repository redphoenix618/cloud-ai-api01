...
public class ALiCloudA2W extends CloudA2W {
    ...
    public ResultCloud audio2Word(String filePath) {
        ...
        // 录音文件公网可访问地址
        // String fileLink = "https://footbaln.oss-cn-hangzhou.aliyuncs.com/audio.mp3?Expires=1655060809&OSSAccessKeyId=TMP.3KeQ3eFWKyAhsJQhZx9k1BTH7C9FnkCVwnWfPoUrRF4NF82FxAYxqFto1XrA2ryxX461v5U998hbfCRcesVre8ELWK2bBy&Signature=JlGQEbB3f0SyAiSVtUK58D8Kzwk%3D";
        ALiCloudA2W demo = new ALiCloudA2W();
        // 第一步：提交录音文件识别请求，获取任务ID用于后续的识别结果轮询。
        String taskId = demo.submitFileTransRequest(appKey, filePath);
        if (taskId != null) {
            System.out.println("录音文件识别请求成功，task_id: " + taskId);
            // return ResultCloud.error("录音文件识别请求成功，task_id: " + taskId);
        }
        else {
            System.out.println("录音文件识别请求失败！");
            return ResultCloud.error("录音文件识别请求失败！");
        }
        // 第二步：根据任务ID轮询识别结果。
        String result = demo.getFileTransResult(taskId);
        Long costTime = DateUtils.costTime(before);
        if (result != null) {
            String content = JSON.parseObject(result).getJSONArray("Sentences").getJSONObject(0).getString("Text");
            ...
            return ResultCloud.error("录音文件识别结果查询失败！");
        }
    }
}
